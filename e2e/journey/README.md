...
Email 提醒、credit 過期機制等），本檔不再另存一份清單。

> 曾列為落差、現已確認為**正式設計**的項目：失效會員「推薦碼不作廢、
> 仍可驗證使用」與「點數/任務保留不歸零、僅擋提領」（規格書 §5.1）；
> 移除寬限期改兩態後「到期即隱藏刊登」（規格書 §5）；**不提供自助取消
> 訂閱**——一次性年費無續扣可停，不續約即到期失效（規格書 §5），因此
> 不需要對應情境；**不提供當月推薦排行榜／排名顯示**——
> `/tasks/current-month-top` 只回傳呼叫者個人本月推薦明細、不提供跨會員
> 排名，同樣不需要對應情境（規格書 §9.1）。
>
> 獎金 100P/代與推薦王門檻 8 人不是落差而是**可調參數**——測試一律以
> `reward_config` 現值計算，調參不改測試。
...
提醒 email、推薦王 credit 的過期機制。（自助取消訂閱已定案不提供，
不列在此。）

尚未落地（後續迭代選項）：
- 刊登編輯流程、`reuse_tree` 快照重用（debug 迭代提速）。
