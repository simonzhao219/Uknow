Feature: 阿凱的七年 — 續約獎勵任務機制的組合行為劇本
  ...
  Scenario: 第 4 章 fresh 換樹清空——A14 揭露、A15 二次確認、U2 首次配對
    ...
    And "K0" 的可提領點數已歸零【DB】
    And "K0" 的任務卡顯示進度 0/8
    And "K0" 的最新到期日為付款日起約一年【DB】
    And "K0" 的獎勵明細出現「新約重置」列
    And "U2" 因 "K0" 的新約獲得第 1 代獎勵【DB】
    And "U2" 的任務卡顯示進度 1/8
    # 現況:W1(K0 在 A 樹收的下線)隨遷——推薦邊仍指 K0、整枝跟進 B 樹。
    # 「另一包」(upline-pairing-lines)上線時此斷言反轉(開放問題 #2a 裁決 (b))。
    And "W1" 的上代仍為 "K0"【DB】
  ...
  Scenario: 第 7 章 S9 與 Q14a——填現任上代碼照樣清空,歷史桶跨清空保留
    ...
    Then "K0" 的上代已改為 "U2"【DB】
    And "K0" 的可提領點數已歸零【DB】
    And "U2" 因 "K0" 的 S9 付款獲得第 1 代獎勵增量 100P 且任務不增加【DB】
