Feature: 提領 — 雙視角全生命週期
  ...
  Scenario: 退件路徑 — 點數退回
    ...
    When "A0" 透過 GUI 申請提領 1000 點
    Then "A0" 的可提領點數減少 1015
    When 管理員在提領管理退件 "A0" 的申請
    # 狀態先於點數：點數是退件的**衍生結果**，先斷言它等於讓「退件根本
    # 沒成功」也報成「點數沒退回」——2026-08-08 run 31263854444 就是這樣
    # 讀成產品可能少退錢的（assert 370 == 1385）。近因先斷言，錯誤訊息
    # 才指得回現場。
    Then 資料庫中 "A0" 最新一筆提領狀態為 "rejected"
    And "A0" 的可提領點數恢復為記下的數值
