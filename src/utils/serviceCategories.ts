 ...
 * 自訂類別長度上限(產品規則)。
 *
 * 判準:**等於內建類別的最長值**(6 字,「各項運動教練」「各類音樂老師」)。
 * 自訂與內建在每個顯示點都並排出現——下拉選單、首頁篩選 chip、四處卡片徽章
 * ——同一個位置卻有兩套長度預期時,長的那套必然先撞牆。`serviceCategories.test.ts`
 * 把這個常數釘在 `SERVICE_CATEGORIES` 的最長值上,所以它不會靜默漂移。
 *
 * 曾經是 10(判準寫「10 個全形字在 375px 的卡片徽章仍能單行呈現」)。那句話
 * 描述的是手機 2 欄詳細卡,但 `DEFAULT_HOME_VIEW_MODE = 'photo'`——使用者
 * 預設看到的是 3 欄照片牆。實測(Chromium,依實際 class 量測):375px 下磚寬
 * 113px、類別膠囊可用 105px,10 個全形字要 122px;320/360/375/390/412 全數
 * 截斷,只有 430px 不截。截斷有 truncate + title 兜底不會破版,但把上限訂在
 * 使用者實際看不完的長度是規則設計錯誤——填滿的人以為別人看得到那 10 個字。
 * 6 字在 375px 只需約 78px,所有常見寬度都完整呈現。
 *
 * ⚠️ 這是**新輸入**的上限,不回頭清既有資料:調降前建立的 7–10 字類別仍在
 * `listings` 裡,照樣會被 `public_listing_categories` 推導出來、出現在下拉
 * 選單與篩選器。顯示端的 truncate + title 本來就為此存在。
 *
 * 資料庫的 trigger 另有一道 20 字的硬上限,那是**濫用上界**不是產品規則:
 * 它防的是繞過這個 UI 的寫入路徑。刻意不跟著調降——它的職責是擋離譜長度,
 * 而收窄它會讓既有 7–10 字類別的擁有者連「編輯刊登的其他欄位」都被擋下。
 */
export const CUSTOM_CATEGORY_MAX_LENGTH = 6;
