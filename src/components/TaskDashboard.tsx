...
export function TaskDashboard() {
  ...
  const handleAcceptReward = (rewardId: string) => {
    ...
    alert(`已成功接受 ${pendingRewards.find(r => r.id === rewardId)?.amount} R點獎勵！`);
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">完成推薦任務，獲得額外R點獎勵</p>
        </div>
      </div>
          ...
          <Card key={task.id} className="relative">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge variant={task.isCompleted ? 'default' : task.currentProgress >= task.maxProgress * 0.7 ? 'secondary' : 'outline'}>
                  {task.isCompleted ? '已完成' : task.isPending ? '待審核' : '進行中'}
                </Badge>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-medium ${getProgressColor(task.currentProgress, task.maxProgress)}`}>
              ...
              <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                ...
                <span className="text-sm font-bold text-yellow-600">{task.reward} R點</span>
              </div>
              ...
              {task.type === 'consecutive_referral' && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {task.details.monthlyReferrals.slice(-8).map((monthData: any, index: number) => (
                      <div key={index} className="text-center p-2 border rounded">
                        <div className="text-xs text-muted-foreground">{formatMonth(monthData.month)}</div>
              ...
              {task.type === 'monthly_yearly_referral' && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="text-sm font-medium">最近推薦日期：</div>
                    <div className="grid grid-cols-2 gap-1 text-xs">
                      {task.details.referralDates.slice(-6).map((date: string, index: number) => (
                        <div key={index} className="flex items-center gap-1 p-1 bg-green-50 rounded">
                          <CheckCircle className="h-3 w-3 text-green-500" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                ...
                <li>• 完成任務獲得1000R點，之後重新計算</li>
                <li>• 推薦成功指推薦的人完成室友刊登並付費</li>
              </ul>
            </div>
