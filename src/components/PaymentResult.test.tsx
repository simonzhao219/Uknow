...
type SlimRenewal = {
  ...
  extendAnchorDate?: string; // 部署交錯期的舊後端沒有這個欄位
...
describe('PaymentResult 補繳中間筆', () => {
  it('completed 且 backfillCount>0 時，顯示補繳進度且不進開通輪詢', async () => {
    ...
    // 進度三要素：已補至（extendAnchorDate − 1 天，formatTwDate 與結帳頁
    // 一致）、還差筆數、總額。
  ...
  it('舊後端形狀缺 extendAnchorDate 時，已補至退回迄日反推一年', async () => {
    // 部署交錯期：前端已上新版、Edge Function 還是舊版精簡契約。
