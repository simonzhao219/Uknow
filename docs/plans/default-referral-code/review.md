...
審查對象:`plan.md` v3.1(commit `2d7c6c8`)|審查日期:2026-07-26
...
**P0 0 項、P1 8 項、P2 4 項。已全數處置 → 規劃書 v4。**

## P1 清單與處置

| # | 視角 | 缺口 | v4 處置 |
|---|---|---|---|
| V3-1 | 系統 | **F 與 H 在 SQL 層無法區分**:`validate_referral_code` 對「碼不存在/未 active」與「推薦人被停權」**都回零列**,v3 的「F 告警、H 不告警」按字面實作做不到,結果會變成停權時每筆付款都告警 | §2.2 改用**只用於告警分類、不參與權限判定**的輔助查詢,兩者都告警但 reason 不同(`default_referrer_suspended` / `default_referrer_code_invalid`)——推薦人被停權導致機制靜默失效同樣是營運必須知道的事 |
| V3-2 | 系統 | **`/auth/register` 第三寫入點的可達性假設未記錄**:它無條件覆寫 `referred_by_*` 卻不碰旗標;之所以無害,純粹靠 `/auth/reset-registration` 的「已完成付款 → 400」守衛擋住編輯入口,而**該守衛目前沒有任何測試保護** | §2.6 新增完整論證;階段 3 補測試**釘住該守衛**,放寬它的人會當場看到紅燈 |
| V3-3 | UI/UX | **抑制只做在渲染層擋不住網路層**:`fetchReferrerInfo`(`PaymentCheckout.tsx:205-271`)早退只查 `referredByCode`,自動綁定者回訪時照樣發 `GET /referrals/validate/<碼>`(回應含推薦人真名)並 `console.log` 印出碼與姓名 | §4.1 抑制**下沉到資料擷取層**,早退條件加 `|| pendingUser.isAutoReferral` |
| V3-4 | UI/UX | `CompleteProfile` 送出前重新驗證會打 `/listings/verify-referral-code`,同樣是網路層洩漏 | **不處理**——經 V3-2 確認該路徑不可達(見下 §4.3 撤回) |
| V3-5 | 架構 | **§5.5 營運手冊會隨 plan.md 在 PR 前被刪除**,規劃書未指定長期落腳處。`docs/supabase-setup-checklist.md` 是精確對應的既有容器(開頭即寫「程式碼與 migration 之外、每環境各做一次的手動設定」,PayUni 憑證等同性質步驟都在那) | 階段 5 交付項 2:明確要求**搬進** checklist,不可只留 plan.md |
| V3-6 | 需求 | §5.5 提領前提清單**漏列 §10.1 檢核 4「會籍在效期內」**,與前半句「累積不需會籍」並列時易誤讀 | §5.5 補完整檢核並明寫「兩者不衝突,是獨立面向」 |
| V3-7 | 需求 | **情境 J 的預期欄沒寫「旗標要重置為 `false`」**——那正是 v3 修的核心 bug,照字面寫測試會漏掉這個斷言 | 情境 J 補斷言 |
| V3-8 | 需求 | **情境 M 漏了第三曝光點**(§4.2 placeholder),測試落點只會覆蓋兩處 | 情境 M 擴為三項(含網路層) |

## P2 清單與處置

| # | 視角 | 缺口 | v4 處置 |
|---|---|---|---|
| V3-9 | UI/UX | §4.3 引用的 `isReferralCodeAcceptable`(`:445`)**在現行檔案不存在**——v3 是基於 rebase 前的版本寫的 | §4.3 整節撤回(見下),同時記錄「基於過期行號設計」這個踩過的坑 |
| V3-10 | UI/UX | `GET /referrals/debug/:userId`(`index.ts:3147-3198`)註解寫 admin only 但實際**放行本人自查**,回傳未過濾的 `referredByCode`,可繞過所有渲染抑制 | 記入 §7 邊界(既有缺口,本 feature 未使其更易觸發,但會使其回傳含新的敏感值) |
| V3-11 | 架構 | §5.5 章節編號在兩個整數章之間插小數點,與模板慣例不同源 | 不改(純可讀性,無機械把關;改動會擾動所有交叉引用) |
| V3-12 | 需求 | 階段 5「規格書同步」只有動作項沒有草稿內容 | 階段 5 交付項 3 補完整要點 |

## v4 的一個範圍**縮小**決定:撤回 §4.3

第三輪同時從兩個視角證明 `CompleteProfile` 的抑制不該做:

- **系統視角**證明 `isEditing && isAutoReferral` **不可達**——`handleEdit` 先打
  `/auth/reset-registration`,該端點對任何有 completed 訂單的人回 400 且前端
  不導頁;而旗標為 true 者必然有 completed 訂單。
- **UI/UX 視角**證明 v3 的佐證引用(`isReferralCodeAcceptable:445`)**已不存在**。

寫一段永不觸發的抑制,正是本專案一路在清的死碼模式(`isAutoReferral` 旗標本身
就是死碼出身,V2-12 也批評過同一件事)。**改為把可達性寫成可執行的假設**——
階段 3 補測試釘住那道守衛。

連帶:V3-4 與 §4.4 的「既有缺陷曝光放大」也因同一條路徑不可達而**不成立**,
一併撤回。

## 需人工裁決

UI/UX 提出一項:`AuthPage.tsx:254`、`CompleteProfile.tsx:148`、
`PaymentCheckout.tsx:129` 三處**既有** `console.log('...', profile)` 會印出完整
profile(含新欄位)。非本 feature 新增,LINE 內建瀏覽器不易觸及開發者工具。
**主 session 處置**:§4.1 已要求移除 `fetchReferrerInfo` 內**本 feature 路徑上**
的 log;其餘三處既有 log 屬獨立的一般性衛生問題,不在本 feature 範圍
(與 issue #167 同樣的切分原則)。記於此供人審覆核。

## 處置

- [x] V3-1~V3-8(P1)全數修訂 → plan v4
- [x] V3-9~V3-12(P2)全數處置(V3-11 明文不改,附理由)
- [x] 需人工裁決項:主 session 依既有切分原則處置,記錄於上
- [x] **結論:v4 無未處置 P0/P1,可進入實作**
