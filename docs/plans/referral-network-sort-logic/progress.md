...
| 7 | 切排序載入回饋(`isValidating` 下傳並呈現) | ✅ 綠 | (見 B-4) | `04744bf` |
| 8 | 型別收斂 `@contract` re-export ＋ 移除死欄位(紅燈 = 執行期斷言) | ✅ 綠 | `9a4497e` | `PENDING8` |
| 9 | 規格書回填(以 code 為準)——文件階段,無測試落點 | ⬜ 未開始 | | |

## 目前位置與下一步

**實作進行中**:9 階段已完成 8 個(1、2、3、4+5 合併、6、7、8)。
...
5. **搜尋分頁接到 UI**:顯示「已顯示 X / Y 筆記錄」+「加載更多」
   (文案與版位照抄 `RewardHistory.tsx` L299–320,不發明新模式);
   加載更多以「已取回筆數」為 offset 續接,不重打第一頁 —— 至此
   「符合條件的都必須搜得到」才真正對使用者兌現

6. **切排序不再是無回饋空窗**:`ReferralManagement` 下傳 `isValidating`,
   重新請求期間樹降透明度 + `aria-busy`(既有債,本次一併償還)

7. **死欄位清除 + 型別收斂**:`subtreeLatestJoinedAt` 與 `subtreeMs` 全數移除;
   前端型別從手抄的第二份契約改為由 `@contract` 衍生——實測若只刪契約那份
   而忘了前端,`npm run check` 與 `deno task check` 會「全綠」,防線是單向的

**下一步:Phase 9**(規格書回填)——需 #11a 的回覆才能寫「為什麼預設是最舊加入」。
...
### B-4 【流程疏失】Phase 7 未先觀察紅燈就實作(2026-07-25)

寫完測試後未跑一次就直接接著實作,`npm run check` 第一次跑就是綠的——
**測試沒有被證明有鑑別力**,不符合 tdd-implement「跑測試確認紅」的要求。

**補救**:事後把實作(`aria-busy` + `opacity-50`)暫時還原,重跑該測試確認
`AssertionError: expected null to be 'true'`(1 failed | 17 passed),
證實測試確實會紅,再還原實作。紅燈證據為此次補驗,非獨立 commit。

**教訓**:紅綠循環的「紅」是要**看到**的,不是推論出來的;連續兩個檔案的
編輯很容易一路寫到底。下次在同一階段內連改測試與實作時,中間強制跑一次。

## 框架摩擦

### F-1 後端階段的紅綠燈完全依賴 CI,一輪來回約 3–5 分鐘

沙箱擋 `jsr.io`、無 docker daemon → Deno 側本機既不能型別檢查也不能跑測試
(詳見 B-1)。四個後端階段各需兩輪 CI,實際佔掉這次實作大半的等待時間。
規則檔已預期此情境(pre-commit 把 `deno task check` 降為警告),但**沒有預期
「紅燈證據只能從 CI 讀」帶來的節奏問題**:CI 有 concurrency group,推新
commit 會取消正在跑的那輪,所以綠燈實作寫好後必須**壓著不推**,等紅燈那輪
跑完才能推。這條紀律沒有寫在任何文件裡,是踩到才知道的(`4027c2f` 那輪就是
這樣被取消的)。

→ 建議:`.claude/rules/supabase-functions.md` 補一句「無法本機跑 Deno 測試的
環境,後端 TDD 階段的紅燈證據取自 CI;此時綠燈實作不得在紅燈那輪 CI 完成前
推送,否則 concurrency group 會取消掉唯一的紅燈證據」。

### F-2 誤把「check-in 觸發時間」當成當下時間,取消了一輪健康的 CI

自動 check-in 觸發後,我未查證實際時間就假設「現在大約是 check-in 的排定
時刻」,據此算出某個 CI job「卡了 42 分鐘」,把它取消並重跑。實際跑 `date -u`
才發現該步驟只跑了 68 秒——**一輪完全正常的 CI 被我取消**。

根因:把間接線索(排程時間、對話節奏)當成可信的時間來源。代價僅是多幾分鐘
CI,但同樣的誤判用在別處(例如判斷部署逾時、鎖過期)可能造成實質破壞。

→ 已把「判斷任何逾時/卡住前,先跑 `date -u` 取得真實時間」寫進本 session 的
check-in 指示。值得考慮寫進框架的通則。
