...
>
> **修訂 3**（2026-09-14）：第 2 輪四視角審查（P0×0 / P1×3 / P2×3）回填，
> 業主兩項裁決已併入。第 1 輪回填標記為〔審查回填 P?-?〕，
> 第 2 輪為〔二審回填 R2-?-?〕。
...
〔審查回填 P1-8〕〔二審回填 R2-系統-1〕**既有 `.dark` 的 destructive **兩個** token
一併改成等價 hex**（業主裁決 Q1）：

| 位置 | 現值 | 動作 |
|---|---|---|
| `globals.css:64` | `--destructive: oklch(0.396 0.141 25.723)` | → 等價 hex |
| `globals.css:66` | `--destructive-foreground: oklch(0.985 0 0)` | → 等價 hex |

二審抓到修訂 2 **只轉了一半**：本節自己寫「destructive **全家族**進對比度測試」，
而 §2.1 的 A 組（實心底＋其上文字）正是 `--destructive` / `--destructive-foreground`
這一對——只轉前者的話，階段 2 會在完全相同的位置、完全相同的原因再卡一次。
...
- **G1 灰階對照表完整性**〔二審回填 R2-架構-1，業主裁決〕：
  掃到的每一個灰階 class（`gray-N` / `slate-N` / `zinc-N` / `neutral-N` / `stone-N`）
  都必須在 `ui-ux-guidelines.md` §12 的灰階對照表裡有對應列，**缺列即紅**。
  存在理由：階段 5 的驗收標準本來寫「對照表涵蓋實測 39 處」，但測試落點只有
  `check-document-naming.py` 與 `check-context-budget.py`——**兩者都不驗表格內容**。
  那正是本規劃在 P1-7（`@theme inline` 只驗 key）與 P1-2（border 宣稱驗過卻沒排進矩陣）
  親自點名的「宣稱驗過但沒驗到」同一種失效模式，只是這次發生在文件而非程式碼。
  「39 處都有一列」是可數、可枚舉的宣稱，`check-spec-drift.py` 已立下
  「列舉與文件不一致就紅」的先例。**這一段是 C1–C3 之外的第二類檢查**
  （驗文件與程式碼一致，不是掃違規），輸出訊息分開，不進 baseline。
...
5. **統計數字去色 + 非狀態判準**（上游 §4 第 5 點指名這是「給 S2 機械替換的裁決依據，
   ...
   - (c) **多值分類色**〔審查回填 P1-5，本次新增並裁決〕：
     ...
     **不走 `--chart-*`**，理由：`--chart-*` 的用途是「同一量綱的多個資料序列」
     ...
     ⚠️ **碰撞防呆**〔二審回填 R2-UIUX-1〕：同一個畫面上**已經有另一套灰**——
     `ReferralTreeView.tsx:72` 的 `expired: { dot: 'bg-gray-400', label: '已失效',
     badge: 'bg-muted text-muted-foreground' }`。那裡的灰不是「沒有顏色」，
     是**「已失效」的專屬語意**。所以世代三階灰的具體色值選定時，
     必須與 `--muted-foreground` / `gray-400` 保持**可辨識的亮度差距**，
     否則三代且已失效的節點會變成「頭像灰＋狀態點灰＋badge 灰字」三重堆疊，
     稀釋掉「已失效」原本刻意要傳達的負面訊號。
     這不是假想：該檔 `:84` 的註解寫著「先前是 userId 雜湊色，調色盤與狀態色／
     世代色撞色，容易被誤讀成分類」——**避免世代色與狀態色互撞本來就是這個元件
     明確在意的既有設計目標**，本裁決若不加這條防呆就正好與它反向。
     *S2 驗收站 1 時業主可推翻此裁決——它是本規劃裡視覺影響僅次於藍色退場的一項；
     驗收項目須明列「深色模式下世代頭像灰是否與已失效圓點灰混淆」。*
   - (d) **純裝飾**：icon 底色、卡片點綴
...
〔二審回填 R2-UIUX-2〕**這份 checklist 是常設規範，不是 S2 的一次性驗收工具。**
§12 收錄時措辭必須是「**任何新增或修改狀態視覺呈現時都要跑**」——
WCAG 1.4.1 這條防線抓不到機械把關（`check-color-usage.py` 的 C1–C3 是語法層規則，
判斷不了「這個 badge 是否只靠顏色傳遞語意」，這是技術限制不是規劃疏漏）。
既然只能靠人工，至少要讓讀 §12 的人知道**這條規則不會過期**——
否則 S2 之後任何新增的純色狀態圓點都不會有人攔。

- **S1 本身不執行這份 checklist**（沒有元件用新 token，無可目視）——
  它從 S2 起長期有效。
...
| 2 | 對比度門檻 + `.dark` 的 `--destructive` **與 `--destructive-foreground`** 改 hex〔P1-8、R2-系統-1〕 | 同檔，新增對比度案例（hex→相對亮度→WCAG 比值，純函式） | 淺色與深色**各自**：文字配對（A、B）≥ **4.5:1**；`*-border` 對其 `*-subtle` ≥ **3:1**〔P1-2〕；裸字 token 對 `--background` 與 `--card` 各 ≥ 4.5:1〔P1-3〕。讀到不可解析的色值**直接紅**，不靜默跳過 |
| 3 | `check-color-usage.py` 判定邏輯（C1 具名 class／C2 漸層／C3 原始色值）+ baseline 四條判定 | `--self-test` 表格案例 | 表格至少覆蓋：純 token 通過／`text-blue-600` 違規／`hover:`·`dark:`·`md:` 變體被抓／裝飾漸層違規／功能性漸層在 baseline 內不報／**`bg-[#16a34a]` 任意值違規**〔P1-1〕／**`.ts` 內 `'#16a34a'` 字面值違規**〔P1-1〕／baseline 內不報／超出報／低於報「請收緊」／**孤兒條目報**〔P2-2〕／**C1 減一 C2 加一淨數不變仍報**〔P2-7〕／**3 位數簡寫 `'#000'` 也違規**（`SignaturePad.tsx:60` 實際存在，正則只寫 `{6}` 會漏檢、與聲稱的 14 處對不上）〔R2-系統-2〕／**非色彩用途的 hex-like 字串不誤報**（例：`href="#a1b2c3"` 錨點）——誤收進 baseline 後因為沒有行內豁免就排不掉〔R2-需求-1〕 |
...
| 5 | `ui-ux-guidelines.md` §12（八點 + 灰階對照表 + 三段 checklist） | **`check-color-usage.py` 的 G1 段**〔R2-架構-1〕＋ `check-document-naming.py`、`check-context-budget.py` | framework-check 綠；**G1 保證對照表涵蓋掃到的每一個灰階 class，缺列即紅**（這格因此有客觀綠燈，不是主觀判斷完工）；章節逐條對得上上游 §4 六點＋本規劃新增兩點 |
...
| P1-5 | 世代色（多值分類色）怎麼辦 | **去色走灰階三階**（＋與「已失效」灰的碰撞防呆） | §4.2 第 5 點 (c) 的理由；S2 驗收站 1 可推翻 |

### 6.3 第 2 輪裁決（2026-09-14）

| # | 問題 | 裁決 | 落點 |
|---|---|---|---|
| R2-架構-1 | 灰階對照表的完整性要不要機械把關 | **機械化**，新增 G1 段 | §2.4 G1、階段 5 |
| R2-架構-2 | 回填後 S1 增重，分工要不要調 | **不拆 session／分支／PR，記異動，實作仍用 Sonnet** | `progress.md` 與上游 `construction-plan.md` 各記一行異動；明寫「實作可能需要兩次對話、中途 `/clear` 續作屬預期內」——框架本來就為此而設（`progress.md` 是 rehydrate 起點） |

不拆的理由：S2 同時依賴 token 與守門腳本，拆開只是把依賴從 session 之間
搬到 PR 之間，總成本不變而 PR 開銷加倍。
...
| C3 誤抓非色彩用途的 hex-like 字串 | 階段 3 自測表格含 true-negative 案例；誤收進 baseline 後**排不掉**（無行內豁免），所以要在寫正則時就擋住，不是事後補〔R2-需求-1〕 |
| 世代灰與「已失效」灰在推薦樹裡撞在一起 | §4.2 第 5 點 (c) 的碰撞防呆（亮度差距要求）＋ S2 驗收站 1 的明列驗收項〔R2-UIUX-1〕 |
