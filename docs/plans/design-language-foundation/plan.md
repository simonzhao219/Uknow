...
>
> **修訂 4**（2026-09-14）：補 §2.3.1「對比度公式必須先錨定參考值」——
> 規劃 PR（#316）合併後補上的一條，見該節。
> 修訂 3：第 2 輪四視角審查（P0×0 / P1×3 / P2×3）回填，業主兩項裁決已併入。
> 第 1 輪回填標記為〔審查回填 P?-?〕，第 2 輪為〔二審回填 R2-?-?〕。
...
### 2.3.1 對比度公式必須先錨定已知參考值（否則測試在測自己）

階段 2 有一個 TDD 的**自我指涉陷阱**：驗證 token 的測試與被測的
`contrastRatio()` **共用同一個公式**。公式寫錯的話——WCAG 相對亮度有一段
gamma 校正的分段函數（`c <= 0.03928 ? c/12.92 : ((c+0.055)/1.055) ** 2.4`），
抄錯門檻值或漏掉分段都不會報錯——測試照樣全綠，但它證明的是
「**這些色值在我的錯公式下達標**」，不是「達標」。

這與本規劃反覆點名的失效模式同源（P1-2 的 border 宣稱驗過卻沒排進矩陣、
P1-7 的 `@theme inline` 只驗 key 不驗值、R2-架構-1 的灰階表宣稱涵蓋卻無落點）：
**閘門說謊比沒有閘門更糟**。差別只在前幾個是「漏驗」，這個是「驗錯」。

所以階段 2 的紅綠循環**分兩段，順序不可顛倒**：

1. **先錨定公式**——用與本專案 token 無關的已知值斷言：

   | 斷言 | 期望值 | 依據 |
   |---|---|---|
   | `#000000` 對 `#ffffff` | **21:1** | WCAG 定義的上界 |
   | 任一色對自己 | **1:1** | 下界 |
   | `#777777` 對 `#ffffff` | **≈4.48:1** | 常被引用的「差一點點不過 4.5」臨界案例 |
   | `#767676` 對 `#ffffff` | **≈4.54:1** | 同上，剛好過 |

   最後兩條是刻意選的：它們**跨過門檻**，公式若有系統性偏差（例如漏掉
   gamma 校正），這兩條會分別落到門檻的錯誤一側，而 21:1 與 1:1 這種
   極端值反而可能僥倖通過。

2. **公式綠了才驗 token**——§2.1 表格的三種形狀、兩種門檻、淺深兩版。

第 1 段沒綠之前，第 2 段的綠燈**沒有意義**。
...
| 2 | 對比度門檻 + `.dark` 的 `--destructive` **與 `--destructive-foreground`** 改 hex〔P1-8、R2-系統-1〕 | 同檔，新增對比度案例（hex→相對亮度→WCAG 比值，純函式） | **分兩段，順序不可顛倒**（§2.3.1）：**(2a) 先錨定公式**——`#000/#fff` = 21:1、同色 = 1:1、`#777/#fff` ≈ 4.48:1（不過）、`#767676/#fff` ≈ 4.54:1（過）；**(2b) 公式綠了才驗 token**——淺色與深色**各自**：文字配對（A、B）≥ **4.5:1**；`*-border` 對其 `*-subtle` ≥ **3:1**〔P1-2〕；裸字 token 對 `--background` 與 `--card` 各 ≥ 4.5:1〔P1-3〕。讀到不可解析的色值**直接紅**，不靜默跳過 |
...
| 風險 | 緩解 |
|---|---|
| **對比度公式寫錯，測試卻全綠**（測試與實作共用同一個函式，測的是「在錯公式下達標」） | 階段 2a 先用與本專案無關的已知參考值錨定公式（含兩個**跨門檻**的臨界案例），綠了才進 2b 驗 token——見 §2.3.1 |
