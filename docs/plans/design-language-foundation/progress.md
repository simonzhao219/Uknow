...
`/review-plan` 第 1 輪完成（`./review.md`）：**P0 × 0、P1 × 9、P2 × 10**，無阻擋項。
業主 2026-09-14 裁決四項（Q1+P1-8 / Q4+P1-4 / P1-9 / P1-6），全採推薦方案。
**P1 與 P2 已全數回填進 `plan.md`**（修訂 2，以〔審查回填 P?-?〕標記）。

**現在：重跑 `/review-plan` 驗證回填品質**——回填本身也是規劃產出，
自己寫完自己看不出問題（這正是 review-plan 存在的理由）。

第 2 輪審查結果出來後**停等人審**。核准後由**業主親自打
`/tdd-implement design-language-foundation`** 啟動實作（那道鎖是
「人審通過才實作」的唯一保證）。

回填後的階段表有兩處實質變動，實作時要注意：
- 階段 1 多驗一件事（`@theme inline` 的**值指向**，不只 key 存在）
- 階段 2 多兩組對比斷言（border 走 3:1、裸字對 `--background` 與 `--card` 各驗）
  並含 `.dark --destructive` 的 oklch→hex 轉換
- 階段 3 的 C3 規則（原始 hex 字面值 + 任意值語法）與 baseline 的
  `{c1,c2,c3}` 分列，是審查新增、原規劃沒有的
