...
**兩個根因都已修、已補防線,並且都已在真後端複驗通過。本檔可以結案。**
...
| **31234221750** | 08-08 02:06–02:28 | **11 failed / 84 passed** | 根因一的複驗 |
| **31235468231** | 08-08 02:40–03:02 | **10 failed / 87 passed** | **根因二的複驗——六條刊登情境全綠** |

| 情境 | 修法前 | 31234221750 | 31235468231 |
|---|---|---|---|
| f40 A0 透過 GUI 建立刊登 | FAILED | ❌ 仍 FAILED | ✅ **PASSED** |
| f40 訪客可在公開首頁搜尋到 | FAILED | ✅ PASSED | ✅ PASSED |
| f40 一個帳號僅能有一筆刊登 | FAILED | ✅ PASSED | ✅ PASSED |
| f40 下架後訪客找不到刊登 | PASSED | ✅ PASSED | ✅ PASSED |
| f60 刊登可見性隨會籍狀態變化 | FAILED | ❌ 仍 FAILED | ✅ **PASSED** |
| f60 過期補繳 | FAILED | ❌ 仍 FAILED | ✅ **PASSED** |

**中間那一欄是這次診斷最有價值的一格**:根因一修好之後,只有「整頁重載才讀
刊登」的情境轉綠,而「同一個 SPA session 內建立完直接導頁」的三條原樣還在
——根因二因此被**單獨隔離**出來(§2b),不必再猜。兩個根因疊在一起時,
第一個會完全蓋住第二個;逐個剝開比一次想清楚兩個容易得多。
...
- ✅ **真後端複驗完成**:根因一見 run 31234221750、根因二見 run 31235468231
  (六條刊登情境全綠,見 §0)。本機不得跑 journey,這是唯一能證明的方式。
