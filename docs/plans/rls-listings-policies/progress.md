...
本 feature 新增 **11 條**情境。既有總數不做精確宣稱——手數 `Scenario:` 會漏掉
`Scenario Outline` 的展開(見 B8-4)。
...
### B8 —— 實作審查抓到的四項偏離(全部已處置)

四視角審實作 diff:**P0 = 0**。以下是未記錄的偏離,逐條補記:

**1. 偽造目標用 B8——規劃書沒寫過這個節點(系統/架構/需求三個視角都抓到)**
plan §2 只定義 B5/B6/B7。實作在情境 7/8/11 額外用 **B8** 當偽造 insert 的目標,
理由是 `listings.user_id` 有 unique 約束(`20260620000001:101`)——拿已有刊登的
節點當目標,RLS 真的破了會撞唯一鍵而不是成功建列,斷言會因為**錯的理由**而過。
理由成立但當時沒套用 plan §2 的撞名查證方法論,也沒記錄。**現已補查證**:
`grep '"B8"'` 全 `e2e/journey/features/` 與 `steps/` 只有本 feature 具名引用
(B8 在 `orgchart.yaml` 只是 `30_tasks` 的 `B1–B8` 集合成員,不會產生刊登),
「B8 名下無刊登」的前提成立。

**2. 訪客 insert 的斷言被放寬——已收緊回 plan 原意(系統/架構/需求)**
plan §1 情境 8 明訂比照情境 7 釘 RLS 形狀,而我實作成三種被拒形狀都算過,
理由寫「anon 的 INSERT GRANT 是環境相依事實」。那是**把 L1 的顧慮誤搬到 L2**:
L2 只在 hosted 分支跑、從不在本地跑,而 plan §2 已實測 hosted 上 anon 對
`listings` 的 INSERT GRANT = true,必然走到 RLS 才被拒。放寬等於放掉
「RLS 是唯一列級授權邊界」這個核心主張。**已改回 `denied_by_rls`**,
Gherkin 步驟一併改成「該次寫入被 RLS 拒絕」,與情境 7、11 對稱。

**3. `EditServiceProvider` 的 §9 註解把保護機制講錯——已改寫(系統/架構/需求)**
我寫「真正擋住他人刊登的是 ownership 檢查的 redirect」,但那支 UPDATE 本身
就帶 `.eq('user_id', user!.id)`(第 266 行)。**git 已確認該行是 develop 上的
既有程式碼**(本 PR 對這個檔案只加了 `//` 註解行,零可執行語法變動),所以
不是未授權的行為變更——而是 **plan 與三輪規劃審查從頭就把這裡的機制描述漏了
一層**,差點被我固化成永久註解。這與 review-v3 的 UI/UX P1 是同一類風險
(寫錯機制的安全論證比不寫更糟),只是那次是寫反、這次是遺漏。
**已改寫**:直接防線是查詢自身的 `user_id` filter;redirect 擋的是另一個
曝險(他人資料被讀進表單顯示),兩者不可混為一談。

**4. 情境總數 38 不可靠——已停止斷言具體數字(架構)**
手數 `Scenario:` 會漏掉 `Scenario Outline` + `Examples` 的展開
(`20_referral_rewards.feature` 就有一個)。本容器也數不出真值:四個 steps
模組因 B5 的 cryptography/pyo3 問題收集失敗。**結論不受影響**——`MIN_FULL=20`
的判準只需要「真實數字遠大於 20」,而漏算只會讓真值更大。已把
progress/plan 裡的具體數字改成不做精確宣稱。

### 其餘實作審查發現(不改程式,記錄即可)

- **6 條驗證標準 → 7 支 `Deno.test`**(需求 P2):內容無增減,是把「逐條角色
  範圍」拆成方向相反的兩支以便看失敗訊息。已在測試檔頭註明。
- **UI/UX 視角零缺口**:逐字核對兩段註解的事實主張(`useUserListing.ts:78`、
  `App.tsx:325` 無 `:id`、`EditServiceProvider.tsx:60-64` 查詢無 user_id 限定、
  `68-74` 的 redirect 在 `setServiceProvider` 之前),並確認整個 diff 在那兩個
  檔案裡只新增 `//` 註解行。
- **L1/L2 是否該拆兩個 PR**(架構 P2,需人工裁決):拆了 L1 能更早落地,但
  第三輪架構審查已評估過「四階段要不要拆」並結論不必;維持單一 PR。
