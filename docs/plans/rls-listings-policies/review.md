...
<!-- P0 的處置規則:必須改 plan 並重跑 /review-plan,或由人在此明文豁免。
     ...
     本次 P0 = 0,以下逐條處置 P1/P2。 -->

### P1

- [ ] 系統 P1-1:補「擁有者 PATCH 自己的刊登成功」正面情境
- [ ] 系統 P1-2:補 `is_admin()` 管理員繞過情境(或併入開放問題 1 的裁決)
- [ ] 架構 P1-1:progress.md 補「規劃檔怎麼帶到 `feature/` 分支」的具體指令
- [ ] 架構 P1-2:合併前手動 `workflow_dispatch` 一次 journey 取得真實紅綠證據
- [ ] 需求 P1-1:L1 補「`listings` 欄位集合 = `public_listings` 欄位集合」比對

### P2

- [ ] 系統 P2-1:§1 註明情境 5、6 保護的是「條件被放寬」而非「policy 被刪除」
- [ ] 系統 P2-2:階段 1 驗證標準改為逐條列出期望角色(3 authenticated / 2 PUBLIC)
- [ ] 架構 P2-1:`rest_as_user.py` 與其測試檔命名成對
- [ ] 架構 P2-2:訂正情境數 28 → **37**,同步更新 §6 開放問題 4 的前提
- [ ] 架構 P2-3〔裁決〕:`45_listing_rls.feature` 是否疊加 `@listing`
- [ ] 需求 P2-1〔裁決〕:後續 11 張表的補齊順序(是否 `profiles`/`withdrawals` 優先)
- [ ] 需求 P2-2:改引 0004/0726 為佐證,並更新 0002 檔頭與 `listings.test.ts:219` 的過時註解
- [ ] 需求 P2-3:§0 加註「不含停權分支」
- [ ] 需求 P2-4〔裁決〕:補寫「刊登隱藏 = 公開可見度,非擁有者自身讀取權」

### 規劃書自列的開放問題(§6)

- [ ] 開放問題 1:L1 是否釘條件表達式全文(注意與系統 P1-2 連動)
- [ ] 開放問題 2:`listings_insert_own` 是否收斂到 `authenticated`
- [ ] 開放問題 3:停權維度是否本輪做
- [ ] 開放問題 4:`MIN_FULL` 是否隨 28 → 37 抬高
