...
- [ ] **P0(renewal 缺漏策略)**:□ (a) 兩選項停用+重試 □ (b) fresh 停用、extend 給 fallback
- [ ] **沖銷列分類**:□ 新增專屬分類(建議,連動全鏈) □ 沿用 adjustment 並改文件與 UI 說明
- [ ] **沖銷插入點**:□ 周邊隔離 + 自癒對帳(reviewer 傾向) □ 關鍵路徑
- [ ] **A16 狀態集合**:□ 只擋 `pending` □ `pending` + `awaiting_collection`
- [ ] **「取消提領」**:□ 改措辭(等待審核/聯繫客服) □ 新增自助取消(範圍擴大)
- [ ] **A14 跳轉前確認**:□ 維持純文字揭露(明文記錄) □ 付款鈕前加確認
- [ ] **PR 拆分**:□ 維持一包 □ 階段 1 併發鎖獨立先行 PR
- [ ] **rules.md**:□ 本包收尾明文排除不刪(建議)
- [ ] P1 其餘(5/6/8/12/13)與 P2:□ 全數修訂
