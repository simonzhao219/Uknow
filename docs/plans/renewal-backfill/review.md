...
- [ ] **Q1(併發缺口)裁決**:□ (a) 納入本 feature □ (b) 另開 fix/ 且本 feature 不先上 □ 其他:
- [ ] **Q2(補繳筆數上限)裁決**:□ 依 A1 收斂,不設限 □ 設限,條件:
- [ ] **Q3(逐筆付款體驗)裁決**:□ 維持逐筆,不做簡化 □ 其他:
- [ ] P0-1 `PaymentResult.tsx` 處置:□ 修訂規劃 □ 豁免(理由:)
- [ ] P0-2 PaymentCheckout 接線 處置:□ 修訂規劃 □ 豁免(理由:)
- [ ] P0-3 併發缺口 處置:□ 修訂規劃 □ 豁免(理由:)
