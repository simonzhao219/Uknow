...
> **版本:第 3 版**(2026-08-02)。第 1 輪審查 P0×3/P1×9/P2×4 → 第 2 版;
> 第 2 輪審查 P0×1/P1×11/P2×4 → 本版。人已裁決 Q1=(a)、Q2=不設限、
> Q3=逐筆、P0=方案(b)、Q4=不收斂、Q5=中性文案、Q6=不做召回。
> 版本差異見 §8。**須重跑 `/review-plan`。**
...
> (見 §6 Q7),此處已改為只陳述規則變更本身,不宣稱使用者利益。
...
- [ ] **Q7(新,聚合者於裁決 Q5 時查證發現):extend 對使用者是否永遠是劣勢選項?若是,補繳制的產品意義為何?**

  §0 第 2 版宣稱 extend 換到「原推薦線與週年日」。查證後:
  - **推薦線不受影響**——`/payuni/prepare` 只在
    `renewalMode === 'fresh' && referredByCode` 兩條件同時成立時才改寫
    `profiles.referred_by_*`(`index.ts:1414`)。選 fresh 而**不填**新推薦碼時,
    上代維持原樣。推薦線從來不是 extend 的專屬價值。
  - **週年日在系統中沒有下游作用**——推薦王月份錨定付款時點
    (`20260724000004:113`)、獎勵綁付款事件、提領守衛只看 active 與否。

  逐案比較(每筆皆 NT$1,200):

  | 情境 | extend | fresh |
  |---|---|---|
  | 過期 2 年 1 個月 | 3 筆 3,600,效期至 2027-04-02(剩 **11** 個月) | 1 筆 1,200,效期至 2027-05-01(剩 **12** 個月) |
  | 過期 1 個月 | 1 筆 1,200,效期至 2027-04-02(剩 **11** 個月) | 1 筆 1,200,效期至 2027-05-01(剩 **12** 個月) |

  **fresh 在所有情境下都不比 extend 貴,涵蓋期都不短於 extend**;過期越久
  差距越大(3 倍價格換更短效期)。

  這不推翻任何已拍板的規則(A1-A9 照舊實作),但它意味著補繳制可能是一個
  **設計出來、但沒有人會理性選擇**的路徑,而本規劃九個階段有相當比例在服務它。
  可能的處理方向(**需人裁決,規劃不自行決定**):
  (a) 接受現狀——照原規則實作,把選擇權交給使用者
  (b) 賦予 extend 實質價值(例如補繳年份折價、或補繳完成後給予額外效期)
  (c) 重新檢視是否真的需要「逐筆補繳」,而非單純放寬新約
...
| **Q4/Q5/Q6** | 開放 | 全數裁決並成文 |
| **Q7** | — | **新增**:extend 是否永遠是劣勢選項(查證發現,待裁決) |
