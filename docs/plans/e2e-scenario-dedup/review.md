...
- [ ] **P0-1** 已處置方式:□ 接受「4 條改留」的修訂 □ 改為先補
      `RequireMembershipRoute` 測試再刪(需同時放寬 §2) □ 其他
- [ ] **P0-2** 已處置方式:□ 採架構判(3 條全改留,已修訂) □ 採系統判
      (刪除但移除失效的 C 級描述) □ 其他
- [ ] **需人工裁決 3**:C 級證據的地位 □ 不算證據(維持修訂) □ 算證據
      (§4.8/§4.10 共 3 條可恢復刪除)
- [ ] **Q1**(規劃書 §6):目標達不到的處置 □ 接受實得(現為 ≈300 分/月)
      □ 另開任務處理固定開銷 □ 重新檢視 xdist 否決
- [ ] 是否要求對修訂後的規劃重跑 `/review-plan`:□ 要 □ 不要(豁免理由:)
