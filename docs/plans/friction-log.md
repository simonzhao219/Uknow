...
## 2026-08-07｜漏網｜required check 的身分是「名字 + SHA」,不是「名字 + 這個 PR」

晉升 PR #243 在 `journey-full` / `api-tests` / `e2e-tests` 三軌**皆
in_progress** 時被合併成功,而 required check `ci-ok` 顯示綠。

這不是規則 7 記的那兩種故障(規則沒生效 / 清單漂移)。閘門有被詢問,
**但它回答的是另一個 run 的結果**:晉升 PR 的 head SHA 必定等於 develop 的
head SHA,所以同一個 commit 上一定有兩個 CI run——develop 的 push run
(journey-full 因 event 不符而 skipped,而 skipped 計為通過 → ci-ok 綠)
與晉升 PR 的 pull_request run(journey-full 真的跑 30-90 分鐘)。push run
先跑完、先在該 SHA 上掛出一個叫 `ci-ok` 的綠燈,branch protection 就放行了。

**每一次晉升都會踩到**,不是偶發。已修(見 `.claude/rules/github-actions.md`
規則 7 的第三種故障,兩道防線 + `check-workflows.py` 規則 7b 機械把關)。

三個可複用的教訓:

1. **「skipped 計為通過」是有射程的**。它為純文件 PR 而設,卻被沿用到
   「promotion-only 的 job 在非晉升 run 上 skip」——同一個語意在兩個情境
   下的意義完全相反(前者是「這軌不適用」,後者是「這軌還沒把關」)。
   放寬條件時要問:**這條放寬會不會剛好覆蓋掉某個閘門唯一存在的場合?**
2. **同一個 SHA 出現在兩種強度不同的 run 裡時,弱的那個必須換名字。**
   否則先跑完的那個會冒充後跑完的那個。
3. **診斷時要看「這個綠燈是哪個 run 產生的」,而不是只看顏色。**
   check run 的 `html_url` 帶 run id,兩個 ci-ok 的 run id 不同——當時如果
   有人點進去看,一眼就會發現綠燈來自 push run。

沿革註記:規則 7 原本記了 PR #109 / #205 / #199 三個「CI 未跑完就合併」的
案例,結論是「不是清單漏了誰,是沒人問過閘門」。#243 是同一個症狀的**第四
個案例、但第二種成因**——閘門這次真的問了。症狀相同不代表成因相同,這正是
規則 7 那張「故障 → 症狀」對照表存在的理由,這次替它補上第三列。
