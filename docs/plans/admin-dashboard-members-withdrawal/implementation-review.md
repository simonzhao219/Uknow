...
**裁決（2026-08-02，人審）：採 (a)。已實作**——會員端保留最近 5 筆已完成記錄
（依 completedAt 排序），`completedByAdmin` 為 true 時標示「管理員代為結案
（逾期未查收由平台確認入帳）」。規格書 §10.3 的斷言自此成立。
...
| 5 | UIUX | 撤銷管理員沒有 `AlertDialog`（plan §4 明列的四個危險動作之一） | **已修**（確認框；授予不設確認——授錯可撤回，兩個方向代價不對稱） |
| 6 | UIUX | 會員查詢台手機仍是 8 欄 `Table`，plan §4 明訂「手機優先：搜尋框置頂 + 結果卡片」 | **人審裁決：另開一輪**（完整功能塊，塞進本 PR 降低審查品質） |
| 7 | UIUX | M3 的狀態篩選與排序 UI 完全未實作（後端已支援 `p_status`／`p_sort`） | **人審裁決：另開一輪**（同上） |
| 8 | UIUX | 統計卡未重用 plan §4 指名的 `ui/stat-card-grid.tsx`；搜尋框缺 `type="search"` | **已修**（兩張統計卡列改 `StatCardGrid`；`type="search"` 補上） |
| 9 | UIUX | 證件審核佇列無分頁也無「已顯示 X / Y」，後端預設 50 —— backfill 上線當下可能一次產生數十筆，超過就靜默截斷 | **已修**（`IdReviewQueue` 改走 `usePagedList`，含「已顯示 X / Y」與載入更多） |
...
- [x] 人審完成，裁決：**修訂後通過**
  - P0-1、P0-2 均已修復（P0-2 依人審裁決採選項 (a)）
  - P1 #1–#5、#8、#9 已修；#6、#7（手機卡片版面、M3 篩選排序 UI）依人審裁決
    **另開一輪**——完整功能塊，塞進已 46 檔的 PR 會降低審查品質
  - P2 五項記入 friction-log 候選，不阻擋本次合併
