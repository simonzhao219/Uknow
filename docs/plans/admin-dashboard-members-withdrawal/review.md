...
**全部 P0/P1/P2 已在 plan.md v3 修訂，無豁免項。** 詳見 plan.md §8 修訂紀錄。

- [x] **P0-1 `withdrawal_events` RLS/GRANT** → 修訂 plan（v3 §2.2 建表即
      `enable RLS` + `revoke all`，並在階段 2.3 驗證標準加「以 authenticated
      身分直讀應失敗」的斷言）
- [x] **P0-2 新函數 `revoke execute`** → 修訂 plan（v3 獨立成 §2.5，列出適用
      清單與 PostgREST 繞過 middleware 的完整攻擊鏈；階段 1.3／3.3 的驗證標準
      明列「以 authenticated 身分直呼 rpc 應失敗」，用測試釘死）
- [x] **P0-3 證件審核是否阻擋提領** → **需求方裁決：只擋已被退回（`rejected`）
      的人**。理由：真正的關卡是匯款不是申請，admin 本來就不會在沒核對證件的
      情況下轉帳；在申請端擋「還沒輪到審核」的人不增加實質保護，只讓每個新
      會員的首次提領多等三個工作天。守衛採最小改動（`none`/`pending`/`approved`
      一律落到現行的照片存在檢查），既有會員零影響。
      **連帶消解四條 P1**：不必解耦提領 dialog、四契約不適用、backfill 從最高
      風險降為低風險整理、沒有「首次提領新等待」。

**需人工裁決 5 項的結果**：
1. §5.3 → 改寫成一般性原則，不逐條加例外
2. admin 代改銀行資訊 → **需求方裁決：不能，一律退件**（已寫進 §1.4）
3. `withdrawal_events` 的 `on delete cascade` → 維持（改 restrict 會破壞既有
   `/auth/delete`），技術債記在 §2.2
4. `admin_review_id` 轉換表 → 已定義（`approved → rejected` 允許、
   `rejected → approved` 需會員重新上傳；不連動既往提領）
5. P0-3 → 見上

**審查期間取得的其他需求方裁決**（已回寫 plan.md）：SLA 3 個工作天、詳情面板
遮罩、交易序號選填、手機只鎖「標記已匯款」。

- [x] **人審完成，裁決：☑ 修訂後通過**

  **依據**（如實記錄，非代填）：使用者於 2026-08-02 合併 PR #186（含 v3 規劃書
  與本審查報告），隨後**親自執行 `/tdd-implement admin-dashboard-members-withdrawal`**
  啟動實作。`/tdd-implement` 是本專案唯一不能自動觸發的 skill，其啟動本身即
  「人審通過才實作」的機制保證。
  **未重跑 v3 審查**——下方建議已於 PR 合併前兩度提示，使用者選擇直接開工。
