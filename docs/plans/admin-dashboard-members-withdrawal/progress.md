...
**已完成 5 階段（PR 1 的後端三階段全綠）。**

| 階段 | 紅燈 | 綠燈 | CI 判定 |
|---|---|---|---|
| 2.1 CSV 跳脫 | `a7669e0` | `bc25432` | 本機 vitest 15 斷言 |
| 2.2 剪貼簿 utility | `499cfe0` | `45dd17d` | 本機 vitest 6 斷言 |
| 1.1 證件審核資料層 | `7523d0e` | `d0b31bd` | api-tests 全綠 |
| 1.2 提領守衛 #5a | `ee6b979` | `70aa55c` | 紅 `173/2` → 綠 `175/0` |
| 1.3 admin 審核端點 | `c8c5c05` | `51e401b` | 紅 `176/9` → 綠 `185/0` |

**進行中：B3**（換照片時審核時間戳未歸零，紅燈 `1267228` 待 CI）。

**接著**：階段 1.4（會員端證件狀態區塊，vitest jsdom）。1.4 起的前端階段
**本機可完整紅→綠**，不必每循環等兩輪 CI，節奏會明顯變快。
