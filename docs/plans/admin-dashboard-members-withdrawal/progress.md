...
| 2.6 | 退件理由端到端 | 🟡 紅燈已推，待 CI 判定 | `98c71e4` | |
...
進行中：階段 2.6，紅燈 `98c71e4` 已推、待 CI 判定。三條斷言分別釘住
「退件理由到得了會員面前」「代為結案要誠實揭露」「本人查收不得誤標」。
預期紅燈形狀：既有 202 支不動，新增 3 支全紅（`row.note` / `row.completedByAdmin`
皆為 `undefined`）。

剩餘：2.6（實作）、2.7–2.9、3.1–3.4、4.1。
