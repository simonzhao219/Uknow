...
| 代數色標（左側 rail） | 計算 | ✅ |
| **最近訂閱到期日**（右側一律顯示，各狀態皆有日期） | `account_status.end_date` | ✅ |
| 狀態燈號（綠/琥珀/灰/紅） | `status` + `suspended_at` | ✅ 失效/停權**只靠燈號**，不加 inline label |
| inline label「剩 N 天到期」 | `daysToExpiry` | ✅ **僅即將到期**才顯示（琥珀 label） |
...
| 即將到期 | active 且距到期 ≤30 天 | 琥珀 | ● | 到期日（+ inline label「剩 N 天到期」） | 可看 | 正常，流失風險高 |
| 已失效 expired | 過期／從未訂閱 | 灰 | ○ 空心 | 到期日（過去日期） | ❌ 標「已下架」 | 停止 |
| 停權 suspended | `suspended_at` 有值 | 紅 | ● | 到期日 | ❌ 標「已下架」 | 領取被擋 |

**樹列格式規則**：
- **右側一律放到期日**（每種狀態都有 `end_date`——失效者為過去日期、停權者訂閱可能仍未到期）。
- **失效／停權不加 inline label**——燈號顏色已足以表達，避免與右側重複。
- **只有「即將到期」加 inline label「剩 N 天到期」**（琥珀），做為斷崖前的續約提醒。
