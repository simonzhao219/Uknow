...
| 網絡累計收益（已入帳） | `reward_schedules` beneficiary=我, completed | ✅ 卡內 slim 一行 |
| 本月待入帳 | `reward_schedules` pending 本月 | ✅ 同上 |
...
| 欄位 | 來源 | 決策 |
|---|---|---|
| 大頭 + 姓名 + 狀態徽章 + 代數徽章 | `profiles` + `account_status` | ✅ |
| 加入日期 | `referral_edges.referred_at` | ✅ |
| 刊登：服務類別、城市、（刊登名 + 連結） | `listings` | ✅（深代數規則見 §3.4） |
| 訂閱到期日 | `account_status.end_date` | ✅ |
| 直接下線數 + 「展開其下線」捷徑 | `childCount` | ✅ |
| 獎勵：已入帳 / 本月待發 / 剩餘月數 + 12 期進度條 | `reward_schedules` 聚合 | ✅ |
| 推薦碼 + 複製 | `referral_codes` | 🟡 僅一代顯示（見 §3.4） |
| 逐月獎勵排程（12 期逐筆） | `reward_schedules` 逐筆 | ❌ 本期不做，留 M4 加值 |
...
- 獎勵：本月已入帳、待發放、剩餘月數 + 12 期進度條。逐月排程明細本期不做（留 M4）。
- 下線數與「展開查看其下線」捷徑。
- 推薦碼：僅一代顯示（見 §3.4）。
...
- 卡片內置頂：**推薦碼 + 分享**（見 §3.5）。
- 其下一行 **slim 網絡收益**：累計已入帳、本月待發放。
- 刻意**不放**本月新增下線、任務進度等激勵型資訊，避免稀釋「看網絡結構」的主任務。

### 3.4 隱私與遮罩規則（跨畫面）
下線分兩層對待：
- **一代（直推）**：你親自找來的人 → **全顯**姓名、刊登、推薦碼。
- **二、三代（下線的下線）**：你未必認識 → **姓名部分遮罩**（如「陳○玲」）、保留服務類別／城市供辨識、**不顯示推薦碼與聯絡方式**。

理由：平衡「看得懂網絡結構」與「不過度暴露非直接關係者的個資、避免跨線騷擾」。遮罩在**伺服器端**完成（API 依觀看者與目標的代數關係決定回傳粒度），前端不持有未遮罩資料。

### 3.5 推薦碼 → 分享（對齊 Dashboard）
卡片內的推薦碼區塊**移除單純「複製推薦碼」**，改為一顆「分享邀請」，直接沿用既有的 `shareReferralInvite()`（`src/utils/referralInvite.ts`）：
...
- **密度＝精簡**（手機維持單行）：狀態點 · 姓名 · 代數色標 · 下線數 pill（如「3 位下線」）· **最近訂閱到期日**（近期到期以警示色）。獎勵摘要、服務類別／城市不放列上，收到詳情。
