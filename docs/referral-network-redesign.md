...
**A. 頂部 · 網絡總覽**（保持乾淨，只留結構與收益）
| 欄位 | 來源 | 決策 |
|---|---|---|
| 我的推薦碼 + 複製/分享 | `referral_codes`(我) | ✅ |
| 三代人數（各代 total / 有效） | 分層 `COUNT` | ✅ |
| 網絡累計收益（已入帳） | `reward_schedules` beneficiary=我, completed | ✅ |
| 本月待入帳 | `reward_schedules` pending 本月 | ✅ |
