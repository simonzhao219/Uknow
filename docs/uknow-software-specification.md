...
| 模組 | 元件 | 功能 |
|---|---|---|
| **會員管理** | `MemberManagement` | 會員列表（全站統計／狀態篩選／排序；列上動作＝查看＋停權/解除停權）、會員詳情（含近期提領記錄，身分證與銀行帳號遮罩；**管理員授予／撤銷只在這裡**，兩個方向都走確認框）、**停權/解除停權**（§5.2，停權走確認框）；次分頁「證件審核」＝身分證照片的通過／退回流程 |
...
「授錯了撤回即可」只在權限層成立。這是 UI 兩個方向都要確認框、且把這顆鍵
移出會員列表的理由（互動位階見 `ui-ux-guidelines.md` §11）。
