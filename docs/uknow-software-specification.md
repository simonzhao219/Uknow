...
- **綁定對象**：會員（User），而非刊登。
- **編碼格式**：數字流水號，由 `referral_code_seq` 單調發出，**8048876 起**
  （8048876、8048877、8048878……）。起始值是結構事實、非可調參數，開始發碼
  後不得 restart；交易 rollback 會消耗掉一個號，序號因此可能有極少數空洞。
- **發碼時機**：首次付款成功時產生。同時僅一個 `active` 碼；續約／補償沿用
  既有碼，完全沒有時才產生新碼——序號只消耗在真正付費的會員身上。
- 第一個發出的碼 `8048876` 同時是 §7.4 預設推薦人的指定碼。
- **比對規則**：碼不分全形半形、不分大小寫——`８０４８８７６` 與 `8048876` 是
  同一個碼（舊格式碼的全形英文同理）。所有輸入點一律先正規化
  （NFKC → 小寫 → 去頭尾空白）再比對。全形是中文輸入法的標準模式，而數字碼
  短、可口述，手動輸入是主要路徑；少了這步，全形碼在系統中與「使用者真的
  打錯碼」完全同形（都只是查無此碼），任何閘門都分不出來。

〔實作〕`20260914000001_referral_code_numeric_sequence.sql`、`generate_referral_code()`、
`src/utils/referralCode.ts` 的 `normalizeReferralCode()`
...
引用計數就是 `group by` 本身。另存一張類別表才需要 trigger 或排程回收，
而那會產生第二份真相（孤兒類別、漏刪、競態）。

推論一：判準是「目前**可見**的刊登在用」，不是「資料列存在」。
`public_listings` 以 `has_active_subscription()` 排除會籍過期與停權者，
所以他們的刊登若是某類別的最後一筆，該類別會暫時消失、續約或解除停權後
自動回來。與 §5.3「刊登可見性用同一把尺」一致。

推論二：類別名稱在寫入時正規化（去頭尾空白、內部連續空白收成一個），
由 `listings` 的 `listings_normalize_category` trigger 強制——
`group by` 是逐字元比對，不正規化就會長出兩個看起來一樣的類別。
...
**授予在資料層面是不可逆的**，方向和直覺相反：管理員當下就讀得到全站會員的
身分證字號與收款帳號（提領作業台維持全碼），撤回權限撤不回已經被看過的資料。
「授錯了撤回即可」只在權限層成立。這是 UI 兩個方向都要確認框的理由。

授予／撤銷與停權／恢復都**只在詳情面板裡**：兩者同屬「對一個人的狀態做判斷」，
做之前該先看清楚他是誰。分類與位置的判準見 `ui-ux-guidelines.md` §11。
