...
重要——而實際使用頻率往往差好幾個數量級。兩個軸決定位階：

| | 破壞力低 | 破壞力高 |
|---|---|---|
| **常用** | 列上，主要視覺權重（`outline` 以上） | 列上，但要一道確認框 |
| **罕用** | 列上或收進詳情，皆可 | **不要放在列上**——移進詳情面板或次級入口 |

三條規則：

1. **視覺權重要跟著使用頻率走，不是跟著危險程度走。** 反例：`MemberManagement`
   改版前每天要按的「查看」是最輕的 `ghost`，偶爾才用的「暫停」卻是滿版紅底
   `destructive`，在掃描時最搶眼——權重和頻率完全相反。危險動作用**紅字**
   （`ghost` + `text-destructive`）就足以讀出危險，真正的防線是確認框，
   不是把它做成視線磁鐵。
2. **罕用＋高破壞力的動作移進詳情面板。** 這不是「藏起來」：這類動作
   （授予後台權限、刪除帳號）本來就需要先看清楚對象是誰，而那些資訊就在
   詳情面板裡——「必須先開詳情」是流程本身，不是人工加的關卡。
3. **溢出選單（`⋯`）要放得下兩個以上的項目才值得。** 只裝一項的選單是把
   一次點擊變兩次而沒換到任何東西。且**時效性動作不要收進選單**：既有先例
   是 `WithdrawalManagement.tsx:167-169`「退件與代為完成不鎖——那是客服接到
   電話當下就該能處理的事」，停權同屬這一類。

**確認框的文案一律說出後果，不是「確定嗎」**——要判斷的是這件事會對那個人
造成什麼，不是重複一次自己剛按了什麼。反向也要守：可逆的動作不收這道過路費
（停權要確認、恢復不要），否則確認框會被訓練成無腦點掉的一步。

**可逆性要看資料層，不只看權限層。** 授予管理員在權限層可逆（隨時撤回），
在資料層不可逆（他已經讀過全站身分證與收款帳號）——判斷「這個動作要不要
確認框」時取兩者中較嚴的那個。
